...
1. **Account Abstraction (EIP-4337):** Every agent gets a smart contract wallet (AgentAccount) via CREATE2. Gas sponsored by AgentPaymaster, agents never hold ETH. EIP-4337 handles bond deposits (`USDC.transfer(escrow)` via UserOp); join/bid actions flow through the Durable Object sequencer (HTTP/MCP) and are batched on-chain via `ingestEventBatch()`. `recordBond` remains a separate admin transaction in MVP (retryable, idempotent on tx hash, see Limitation #4).
2. **Identity & Privacy (ERC-8004 + ZK sidecar):** The official ERC-8004 `IdentityRegistry` is the settlement source-of-truth (`ownerOf`, `getAgentWallet`, `setAgentWallet`). Privacy is implemented in a separate sidecar registry (`AgentPrivacyRegistry`) that stores capability commitments for ZK proofs. NullifierSet prevents double-joining/double-bidding.
3. **ZK Verification (Groth16):** RegistryMembership and BidRange verifiers are available on-chain for sealed-bid and optional direct-chain admission paths. MVP join/bid admission verifies proof bytes in the DO sequencer path. Batch event ingestion via `ingestEventBatch()` validates the Poseidon hash chain in memory and persists only the final `chainHead`. Circom 2.x (v2.2.3) circuits compiled with snarkjs (v0.7.5), Hermez Powers of Tau trusted setup.
4. **Auction Logic (Poseidon chain + sealed-bid MPC):** AuctionRoom maintains an on-chain Poseidon hash chain for settlement-critical events. SealedBidMPC handles ElGamal-encrypted bids with 3-of-5 threshold decryption + FROST signatures.
...
Cloudflare Durable Objects handle real-time auction logic (~ms latency); on-chain contracts handle settlement-critical state; CRE handles oracle verification. EIP-4337 path uses direct USDC transfer to escrow; x402 is used for HTTP micropayments (room access, manifest fetching) and EOA fallback deposit flow. Periodic on-chain Poseidon hash anchoring during auctions enables independent verification — CRE reads anchored hashes from chain, not from the platform API.
...
| 06 | [Appendix](./agent-auction-architecture/06-appendix.md) | System Architecture diagram, Contract Deployment Order (15 steps), High-Risk Test Checklist, Technology Stack, Hackathon Execution Plan, Competitive Analysis, Limitations & Caveats (1–19), Bibliography [1]–[39], Methodology |
