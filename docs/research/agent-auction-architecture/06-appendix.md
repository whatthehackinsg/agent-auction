 ...
 **Reorg/finality policy:** Integration tests that simulate a reorged `recordResult` (SAFE) and verify the system re-submits idempotently, while `AuctionEnded` settlement trigger waits for FINALIZED. Confirm settlement uses only the on-chain `finalLogHash` at finality. (⚠ `anchorHash()` is REMOVED in amended architecture — periodic anchoring no longer exists; only the single `finalLogHash` at close is relevant.)
...
5. **Sequencer key is the single most critical security parameter — compromise can steal all pending escrow funds.** If the sequencer private key is compromised, the attacker can: (a) declare fake winners via `recordResult()` pointing to attacker-controlled wallets with fabricated `finalLogHash` and `replayContentHash`, (b) CRE would validate the fabricated data if the attacker also controls the IPFS-pinned replay bundle (consistent-but-false bundle) — **CRE provides zero protection against a compromised sequencer that controls both `recordResult` and the replay bundle**. The attacker can drain every pending auction's escrow in a single block. `setSequencer()` (owner-only) can rotate the key, but a compromised sequencer can act before detection. **Recovery for already-settled auctions is impossible** — once `_processReport` credits the attacker's wallet and `withdraw()` is called, the USDC is gone. For hackathon MVP: single EOA owner is acceptable, documented risk. (⚠ `anchorHash()` is REMOVED in amended architecture — the attack vector is now `recordResult()` + IPFS bundle, not `anchorHash()`.)
   ...
   **Production Go-Live Checklist (P1):**
   ...
   - **Independent witness node:** A separate, read-only node monitors all `recordResult` transactions (⚠ `anchorHash` monitoring removed — `anchorHash()` no longer exists in amended architecture). Compares result patterns against expected behavior. Alerts if any anomaly. This is the detection layer that enables the pause trigger above.
